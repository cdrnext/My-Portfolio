...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ── AUDIO SETUP ─────────────────────────────────────────────────────────────
  // Try HTML element first; fall back to Audio() if needed
  let bgAudio = document.getElementById('bgAudioEl');
  ...
  // Verify the src is correct (for debugging)
  console.log('[Music] Audio element ready. src:', bgAudio.src || bgAudio.currentSrc);

  function setMusicPlaying(state) {
    isAudioPlaying = state;
    if (musicToggleBtn) {
      musicToggleBtn.classList.toggle('playing', state);
      musicToggleBtn.setAttribute('title', state ? 'Pause Music' : 'Play Music');
    }
  }
  ...
  function toggleMusic() {
    ...
    if (isAudioPlaying) {
      ...
      }).catch(() => showToast('🎵 Tap screen to play music'));
    }
  }
...
DESCRIPTION:You are warmly invited to celebrate Manoj & Bhagya's Homecoming Night Party! Let's Reunite\, Celebrate & Make Beautiful Memories Together! Map: https://share.google/6J5byoG0V9HshOmCD RSVP: https://forms.gle/v9hv6yD79fJcLnSB6
LOCATION:Royal Rest House\, Peradeniya
...
END:VCALENDAR`;
  ...
  document.addEventListener('click', (e) => {
    const galleryImg = e.target.closest('.gallery-item img, .story-photo-box img, .hero-banner-box img');
    if (galleryImg) {
